...
try{
    ...
    $handle_pie = $link->prepare("SELECT Count(school_year.school_year) as y, UPPER(CONCAT(feeder_data.feeder_type, ' ', 'SHS')) as x FROM school_year INNER JOIN feeder_data ON school_year.school_year_id = feeder_data.feeder_school_year Where school_year.school_year Like '%SHS%' GROUP BY feeder_data.feeder_type"); 
    $handle_pie->execute(); 
...
chart.options.data = visitorsData["Public Vs Private SHS"];
chart.render();
