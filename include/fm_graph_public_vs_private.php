...
try{
    ...
    $handle_pie = $link->prepare("SELECT Count(school_year.school_year) as y, UPPER(CONCAT(feeder_data.feeder_type, ' ', 'FRESHMEN')) as x FROM school_year INNER JOIN feeder_data ON school_year.school_year_id = feeder_data.feeder_school_year Where school_year.school_year Like '%F%' GROUP BY feeder_data.feeder_type"); 
    $handle_pie->execute(); 
...
chart.options.data = visitorsData["Public Vs Private FRESHMEN"];
chart.render();
